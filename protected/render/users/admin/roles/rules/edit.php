...
<body>
    <h1>Users</h1>
    <a href="<?= APP::Module('Routing')->root ?>admin">Admin</a> > <a href="<?= APP::Module('Routing')->root ?>admin/users">Users</a> > <a href="<?= APP::Module('Routing')->root ?>admin/users/roles">Roles</a> > <a href="<?= APP::Module('Routing')->root ?>admin/users/roles/rules/<?= APP::Module('Routing')->get['role_id_hash'] ?>"><?= $data['role'] ?></a> > Edit rule
    <hr>
    ...
    <form id="edit-rule">
        ...
        <br>
        <input id="uri_pattern" type="text" name="uri_pattern" value="<?= htmlspecialchars($data['rule'][0]) ?>" style="width: 500px"> (regular expression)
        <div class="error"></div>
        <br><br>
        
        <label for="target">Target URI</label>
        <br>
        <input id="target" type="text" name="target" value="<?= htmlspecialchars($data['rule'][1]) ?>" style="width: 500px">
        <div class="error"></div>
        <br><br>
        ...
        $('#edit-rule').submit(function(event) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    $('#edit-rule').find('[type="submit"]').attr('disabled', false);
                }
            });
          });
    </script>
