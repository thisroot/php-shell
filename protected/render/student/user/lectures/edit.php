...
<head>
    ...
    <meta name="title" content="Предмет: <?=
    $data['name'] .
...
<!--    <script src="<?= APP::Module('Routing')->root ?>public/ui/vendors/summernote/dist/summernote-updated.min.js" type="text/javascript"></script>-->
    <script src="<?= APP::Module('Routing')->root ?>public/plugins/summernote/dist/summernote.js" type="text/javascript"></script>
        ...
        $(document).ready(function () {
            ...
            function refresh() {
                ...
                $('.js-expand').on('click', function () {
                    var id = $(this).data('id');
                    if (!$('#dd3-content-' + id).hasClass('edit')) {
...
//             MathJax.Hub.Queue(["Typeset",MathJax.Hub,math]);
                ...
                $('.js-save').on('click', function () {
                    ...
                    var id = $(this).data('id');
                    var html_data = $('#html-editor-' + id).summernote('code');

                    if (html_data === '<br>') {
                        $('#html-editor-' + id).summernote('code');
                        html_data = '';
                    }
                    ...
                    $('#html-editor-' + id).summernote('destroy');
            ...
            function apendState(list, index, el) {
                ...
                $.each(list, function (item, value) {
                    ...
                    if (state[1]) {
                        $("#item-" + value.id_block).find('.button-expand').first().trigger('click');
                    }
                    if (state[2]) {
                        $("#item-" + value.id_block).find('.js-edit').first().trigger('click');
                    } else if (state[0]) {
                        $("#item-" + value.id_block).find('.js-expand').first().trigger('click');
                    }
                });
